...
<details> <summary> テスト実行順序について </summary>

本エンジンは複数のテストメソッドを単一のJVM内で順次実行するため、テスト対象が静的フィールドや標準入力（System.in）を共有する場合、実行順序によってカバレッジ計測結果が変動しうる。これを避けるため、テストメソッドはメソッド名の辞書順に固定して実行する。これは計測の再現性を担保するための仕様であり、テスト間の状態共有が存在する場合でも同一の入力に対して常に同一の結果を返す。
